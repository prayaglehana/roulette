...
//helo
var roulette = rouletteContract.at('0xbadbd1f083882245bdee1fc37370e80ddc20c2a4');
...
function draw() {
    ...
    if(start==true && f_is_set==true)
        {   console.log('inc'+inc_theta);
            if(inc_theta<0){start=false;
     //   wheel_sound.stop();
                  }
            
            theta=theta+in_rad(inc_theta);
            
            inc_theta=inc_theta-(f);
...
$("#claimReward").click(function(){
    roulette.claimReward(function(error, result){
        if(!error)
            {   
                console.log('win amount is successfully transferred to you :-)') ;        
            }
        else
            console.error(error);
    });   
